...
-   `Year`: Year the set was produced

The goal of this analysis is to predict the Amazon price based on the year, number of pieces, theme, and size of pieces. We will only include observations that have recorded values for all relevant variables.

```{r load-data}
legos <- read_csv("data/lego-sample.csv") |>
  select(Year, Size, Pieces, Theme, Amazon_Price) |>
  drop_na()
```

-   What is a disadvantage of dropping observations that have missing values? How might it impact the generalizability of conclusions?
...
p_year <- ggplot(data = legos, aes(x = Year)) +
  geom_histogram() + 
    labs(title = "Year of production",
         x = "") + 
...
p_pieces / (p_year + p_size)

```

-   What (if any) feature engineering might we want to include in a recipe for a model for `Year`, `Pieces`, and `Size`?
...
Fit the model using `Pieces`, `Size`, `Year`, and `Theme` to understand variability in `Amazon_Price`. To do so, specify the model and use training data to create a recipe applying the feature engineering steps mentioned above. Then, build the model workflow and fit the model.
