...
You will authenticate GitHub using SSH. Below are an outline of the authentication steps; you are encouraged to follow along as your TA demonstrates the steps. [Click here](https://github.com/DukeStatSci/github_auth_guide) to find more detailed instructions.
...
![](images/01/github-example-2.png)
...
3.  Use the visualization to describe the distribution of price. In your narrative, include description of the shape, approximate center, approximate spread, and any presence of outliers. Briefly explain why the median is more representative of the center of this distribution than the mean.
...
4.  In this course, we'll be most interested in the relationship between two or more variables, so let's begin by looking at the distribution of price by category. We'll focus on the four categories in the code below, since these include commonly purchased types of furniture.
...
::: callout-important
**You will use this new data frame for the next few questions.**
:::

5.  Another way to visualize numeric data is using density plots. Make a density plot to visualize the distribution of `price_usd` by category.Let's make a new visualization with the density curves colored by category, so we can compare the distribution of price for each category.

```{r}
#| eval: true
ggplot(data = ikea_sub, aes(x = width, fill = category)) +
  geom_density() 
```
...
9.  You decide to focus on purchasing a sofa for now, so you want to fit a model using the width of the sofa to predict the price. Create a new data frame called "sofa" that only includes furniture items in the category `Sofas & armchairs`. How many observations are in the `sofa` data frame?

::: callout-important
You will use the `sofa` data frame for the remainder of lab.
:::

Use your visualization to describe the relationship between the width and price of Ikea

10. Visualize the relationship between the width and price of furniture in the sofas & armchairs category. Describe the relationship.
11. Fit a linear model that can be used to predict the price of an item of furniture based on the width.
12. Write the equation of the estimated model from the previous exercise.
13. Interpret the slope in the context of the data. Does the intercept have a meaningful interpretation? If so, interpret the intercept in the context of the data. Otherwise briefly explain why not.
14. You would like to do a preliminary evaluation of the model. Calculate $R^2$ and interpret it in the context of the data. Then, calculate RMSE and explain what this value tells you in the context of the data.
